...
func resourceStorageProfileCreate(d *schema.ResourceData, m interface{}) error {
	log.Printf("Starting to create cas_storage_profile resource")
	...
	log.Printf("Finished to create cas_storage_profile resource with name %s", d.Get("name"))

	return resourceStorageProfileRead(d, m)
}

func resourceStorageProfileRead(d *schema.ResourceData, m interface{}) error {
	log.Printf("Reading the cas_storage_profile resource with name %s", d.Get("name"))
	apiClient := m.(*Client).apiClient
...
func resourceStorageProfileDelete(d *schema.ResourceData, m interface{}) error {
	log.Printf("Starting to delete the cas_storage_profile resource with name %s", d.Get("name"))
	apiClient := m.(*Client).apiClient
	...
	d.SetId("")
	log.Printf("Finished deleting the cas_storage_profile resource with name %s", d.Get("name"))
	return nil
}
