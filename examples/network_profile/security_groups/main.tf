...
data "vra_security_group" "example_security_group" {
  filter = "name eq '${var.security_group_name}' and cloudAccountId eq '${data.vra_cloud_account_aws.aws_cloud_account.id}' and externalRegionId eq '${var.region}'"
}
...
resource "vra_network_profile" "example_network_profile" {
  ...
  security_group_ids = [ data.vra_security_group.example_security_group.id ]
}
