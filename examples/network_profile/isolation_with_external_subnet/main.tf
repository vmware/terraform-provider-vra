...
data "vra_fabric_network" "example_external_subnet" {
  filter = "name eq '${var.external_subnet_name}' and cloudAccountId eq '${data.vra_cloud_account_aws.aws_cloud_account.id}' and externalRegionId eq '${var.region}'"
}
...
resource "vra_network_profile" "example_network_profile" {
  ...
  isolation_external_fabric_network_id = data.vra_fabric_network.example_external_subnet.id
}
